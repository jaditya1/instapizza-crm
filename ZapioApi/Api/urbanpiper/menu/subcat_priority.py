...
class SetSubCatPriority(LoggingMixin,APIView):
	"""
	Outletwise Sub-Catregory Priority SEt POST API

		Authentication Required		: Yes
		Service Usage & Description	: This Api is used to set priority of subcategory outletwise.
	...
	def post(self, request, format=None):
		try:
			...
			query = SubCatOutletWise.objects.filter(id=data["id"])
			if query.count() != 0:
				outlet = query[0].sync_outlet_id
				record_check =  SubCatOutletWise.objects.filter(~Q(id=data["id"]), Q(sync_outlet=outlet),\
