...
def swiggy_menu(company_id, outlet_id, user):
	sync_data = {}
	...
	if product_q.count() == 0:
		error = {
			"status":False,
			"message" : "No products are attached for swiggy menu push!!"
			}
		return error
	else:
		item_ids = []
	...
	# tax_record = TaxSetting.objects.filter(company=company_id,active_status=1)
	# if int(company_id) == 1:
	# 	for i in tax_record:
	# 		p_ids = product_q.filter(product__tax_association__contains=[str(i.id)])
	# 		if p_ids.count() != 0:
	# 			tax_dict = {}
	# 			if i.tax_name == "CGST":
	# 				tax_dict["ref_id"] = str(outlet_id)+"CGST"+str(i.id)
	# 				tax_dict["title"] = "CGST"
	# 				tax_dict["description"] = "CGST on matched items"
	# 			else:
	# 				tax_dict["ref_id"] = str(outlet_id)+"SGST"+str(i.id)
	# 				tax_dict["title"] = "SGST"
	# 				tax_dict["description"] = "SGST on matched items"
	# 			tax_dict["active"] =  True
	# 			tax_dict["structure"] = {}
	# 			tax_dict["structure"]["type"] = "percentage"
	# 			tax_dict["structure"]["applicable_on"] = "item.price"
	# 			tax_dict["structure"]["value"] = i.tax_percent
	# 			tax_dict["item_ref_ids"] = []
	# 			for j in p_ids:
	# 				urban_sync_id = "I-"+str(j.id)
	# 				tax_dict["item_ref_ids"].append(urban_sync_id)
	...
	# tax_dict["title"] = "CGST"
	# tax_dict["description"] = "CGST on all items"
	# tax_dict["active"] =  True
	# tax_dict["structure"] = {}
	# tax_dict["structure"]["type"] = "percentage"
	# tax_dict["structure"]["applicable_on"] = "item.price"
	...
	# tax_dict["title"] = "SGST"
	# tax_dict["description"] = "SGST on all items"
	# tax_dict["active"] =  True
	# tax_dict["structure"] = {}
	# tax_dict["structure"]["type"] = "percentage"
	# tax_dict["structure"]["applicable_on"] = "item.price"
