...
def master_flush_menu(company_id, user):
	outlet_id_record = MenuPayload.objects.filter(~Q(outlet=None),Q(company=company_id)).order_by('-created_at')
	outlet_id = outlet_id_record[0].outlet.id
	...
	sync_data = {}
	sync_data["flush_categories"] = True
				...
				if i.variant == None:
							...
							if addon_grp_q.count()==0:
								...
								if temp_addon_data.count()==0:
									...
									if query.associated_addons != None and \
										...
										addon_grp_dict["clear_item_ref_ids"] = True
					...
					if "104" in i.addpn_grp_association:
						...
						if temp_Ketchup.count() == 0:
							...
							Ketchup_dict["clear_item_ref_ids"] = True
					...
					if 	i.zomato_crust_id != None:
						...
						if temp_addongrp_data.count() == 0:
							product_check_zomato_crust_wise = \
							product_q.filter(addpn_grp_association__contains=[str(zomato_crust_id)])
							addon_grp_dict = {}
							...
							addon_grp_dict["item_ref_ids"] = []
							addon_grp_dict["clear_item_ref_ids"] = True
							for l in product_check_zomato_crust_wise:
								item_id = "Z-MI-"+str(l.id) 
								if item_id not in addon_grp_dict["item_ref_ids"]:
									addon_grp_dict["item_ref_ids"].append(item_id)
								else:
