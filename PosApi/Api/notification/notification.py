...
class orderNotificationSeen(APIView):
	...
	def post(self, request):
		try:
			user = request.user
			outlets_data = outlets(user)
			associated_outlet = outlets_data["outlet_ids"]
			# outlets = outlets(user)["outlet_ids"]
			now = datetime.now()
			todate = now.date()
			time_24_hours_ago = now - timedelta(days=1)
			orderdata = Order.objects.filter(Q(order_time__gte=time_24_hours_ago),\
											Q(outlet__in=associated_outlet))
			today_order = orderdata.count()
			seen_count = orderdata.filter(order_status_id=1).count()
			return Response({
				"status"		:	True,
